...
PHASE_NAME='V10_PHASE3_FIX7_GLOBAL_LIGHT_CORPORATE_LIVE'
PHASE_VERSION='2026-07-03.7'
...
def install(Handler, BASE_DIR, load_latest, *args, **kwargs):
    ...
    def router_rows():
        migrate_fix8()
        with con() as c:
            return [dict(r) for r in c.execute('SELECT * FROM router_isp_links ORDER BY wan_name').fetchall()]
    def users_rows():
        migrate_fix8()
        with con() as c:
