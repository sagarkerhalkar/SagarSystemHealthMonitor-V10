...
# ===== V10 MERGE ADDON: old UI kept + HW/SW/GPU/current machine/inventory bridge =====
# Scope: V10 test server only. Live 2278 client flow and live DB are not changed.
# Reads latest rows from live DB and writes only V10 sidecar JSON files under V10 data folder.
V10_MERGE_VERSION = "V10-merge-old-ui-hw-sw-gpu-inventory-20260627"
try:
    import urllib.parse as _v10_urlparse
    import zipfile as _v10_zipfile
except Exception:
    pass

V10_SOFTWARE_REGISTER_PATH = DATA_DIR / "v10_software_license_register.json"
V10_NOTES_PATH = DATA_DIR / "v10_merge_notes.json"


def _v10_low(v: Any) -> str:
...
def _v10_first_list(payload: Dict[str, Any], paths: List[str]) -> List[Any]:
    for p in paths:
...
def _v10_score_machine(m: Dict[str, Any]) -> int:
    p = m.get("payload") if isinstance(m.get("payload"), dict) else {}
    score = 0
    for k in ["cpu_percent", "ram_percent", "disk_max_percent", "wan_download_mbps", "wan_upload_mbps", "primary_ip", "public_ip", "isp_name"]:
        if m.get(k) not in (None, "", [], {}):
            score += 5
    score += min(200, int(to_float(m.get("software_count"), 0) or 0))
    score += min(100, int(to_float(m.get("usb_count"), 0) or 0) * 4)
    score += min(80, int(to_float(m.get("gpu_count"), 0) or 0) * 20)
    if isinstance(p, dict):
        if _v10_first_list(p, ["software.installed", "software.apps", "software"]): score += 100
        if _v10_first_list(p, ["storage.disks", "hardware.disks", "disks"]): score += 80
        if _v10_first_list(p, ["hardware.gpus", "gpu.devices", "gpus", "gpu"]): score += 80
        if _v10_first_list(p, ["usb.devices", "peripherals", "usb"]): score += 60
        if isinstance(p.get("identity"), dict): score += 30
    return score


def _v10_current_machines() -> List[Dict[str, Any]]:
    """Current-machine concept: one best row per real PC, avoid stale fake serial duplicates."""
    raw = load_latest()
    grouped: Dict[str, Dict[str, Any]] = {}
    for m in raw:
        p = m.get("payload") if isinstance(m.get("payload"), dict) else {}
        host = clean_str(m.get("hostname") or get_nested(p, ["hostname", "identity.hostname", "computer_name"], ""))
        primary_ip = clean_str(m.get("primary_ip") or get_nested(p, ["network.primary_ip", "primary_ip"], ""))
        idsrc = clean_str(m.get("id_source"))
        # Prefer ASSET identity. For bad old rows, group by host+ip so one machine is shown only once.
        if clean_str(m.get("machine_id", "")).startswith("ASSET:"):
            key = "asset:" + clean_str(m.get("machine_id"))
        elif host and primary_ip:
            key = "hostip:" + host.lower() + "|" + primary_ip
        elif host:
            key = "host:" + host.lower()
        else:
            key = "id:" + clean_str(m.get("machine_id"))
        item = dict(m)
        item["v10_data_score"] = _v10_score_machine(item)
        item["v10_current_machine_key"] = key
        item["v10_current_machine_note"] = "deduped current machine from latest live payload; stale fake serial rows hidden when richer row exists"
        old = grouped.get(key)
        if not old or item["v10_data_score"] > old.get("v10_data_score", 0):
            grouped[key] = item
    out = list(grouped.values())
    out.sort(key=lambda x: (not bool(x.get("online")), str(x.get("hostname") or "").lower()))
    return out


def _v10_gpu_rows(machine: Dict[str, Any]) -> List[Dict[str, Any]]:
    p = machine.get("payload") if isinstance(machine.get("payload"), dict) else {}
    raw = _v10_first_list(p, ["hardware.gpus", "gpu.devices", "gpu.gpus", "gpus", "gpu"])
    if not raw and machine.get("gpu_names"):
        names = machine.get("gpu_names")
        raw = [{"name": n} for n in (names if isinstance(names, list) else [names])]
    rows: List[Dict[str, Any]] = []
    for i, g in enumerate(raw):
        if not isinstance(g, dict):
            g = {"name": clean_str(g)}
        name = clean_str(_v10_pick(g, "name", "display_name", "caption", "model", "description"))
        rows.append({
            "machine": machine.get("hostname") or machine.get("machine_id"),
            "machine_id": machine.get("machine_id"),
            "primary_ip": machine.get("primary_ip"),
            "gpu_index": i,
            "name": name or "GPU reported without name",
            "vendor": clean_str(_v10_pick(g, "vendor", "manufacturer")),
            "type": clean_str(_v10_pick(g, "type", "adapter_type")),
            "memory_mb": _v10_pick(g, "memory_mb", "dedicated_memory_mb", "vram_mb", "total_memory_mb", "adapter_ram_mb", default=""),
            "usage_percent": _v10_pick(g, "usage_percent", "utilization_percent", "load_percent", "gpu_usage_percent", default=machine.get("gpu_max_usage", "")),
            "temperature_c": _v10_pick(g, "temperature_c", "temperature", "temp_c", "gpu_temp_c", default=machine.get("gpu_max_temp_c", "")),
            "driver_version": clean_str(_v10_pick(g, "driver_version", "driver")),
            "source": clean_str(_v10_pick(g, "source", default="client_payload")),
            "sensor_note": clean_str(_v10_pick(g, "sensor_note", "note", default="N/A means driver/tool did not expose the value; value is not faked")),
        })
    return rows


def _v10_disk_rows(machine: Dict[str, Any]) -> List[Dict[str, Any]]:
    p = machine.get("payload") if isinstance(machine.get("payload"), dict) else {}
    raw = _v10_first_list(p, ["storage.disks", "hardware.disks", "disks", "storage.volumes"])
    rows: List[Dict[str, Any]] = []
    for i, d in enumerate(raw):
        if not isinstance(d, dict):
            continue
        name = clean_str(_v10_pick(d, "name", "drive", "mount", "letter", "device", "model", default=f"disk-{i}"))
        ...
        rows.append({
            "machine": machine.get("hostname") or machine.get("machine_id"), "machine_id": machine.get("machine_id"), "primary_ip": machine.get("primary_ip"),
            "disk": name, "model": clean_str(_v10_pick(d, "model", "caption", "friendly_name")), "serial": clean_str(_v10_pick(d, "serial", "serial_number")),
            "type": clean_str(_v10_pick(d, "type", "media_type", "bus_type", "kind")), "total_gb": total if total is not None else "", "used_gb": used if used is not None else "", "free_gb": free if free is not None else "", "used_percent": pct if pct is not None else "",
            "health": clean_str(_v10_pick(d, "health", "health_status", "status")), "temperature_c": _v10_pick(d, "temperature_c", "temp_c", "temperature", default=""),
            "firmware": clean_str(_v10_pick(d, "firmware", "firmware_version")), "sensor_note": clean_str(_v10_pick(d, "sensor_note", "note", default="Disk temp/SMART may be N/A if driver/tool does not expose it")),
        })
    return rows


def _v10_memory_rows(machine: Dict[str, Any]) -> List[Dict[str, Any]]:
    p = machine.get("payload") if isinstance(machine.get("payload"), dict) else {}
    raw = _v10_first_list(p, ["memory.modules", "hardware.memory.modules", "ram.modules"])
    rows: List[Dict[str, Any]] = []
    if not raw:
        rows.append({"machine": machine.get("hostname"), "machine_id": machine.get("machine_id"), "component":"RAM", "total_gb": machine.get("ram_total_gb", ""), "used_gb": machine.get("ram_used_gb", ""), "used_percent": machine.get("ram_percent", ""), "slot":"summary", "temperature_c":"", "sensor_note":"RAM module temperature normally not exposed on most client machines"})
        return rows
    for i, r in enumerate(raw):
        if not isinstance(r, dict):
            continue
        rows.append({"machine": machine.get("hostname"), "machine_id": machine.get("machine_id"), "component":"RAM", "slot": clean_str(_v10_pick(r, "slot", "bank", "device_locator", default=str(i))), "capacity_gb": _v10_pick(r, "capacity_gb", "size_gb", "capacity", default=""), "speed_mhz": _v10_pick(r, "speed_mhz", "speed", default=""), "manufacturer": clean_str(_v10_pick(r, "manufacturer", "vendor")), "serial": clean_str(_v10_pick(r, "serial", "serial_number")), "part_number": clean_str(_v10_pick(r, "part_number", "part")), "temperature_c": _v10_pick(r, "temperature_c", "temp_c", default=""), "sensor_note":"RAM temp N/A when motherboard/SMBus does not expose it"})
    return rows


def _v10_cpu_row(machine: Dict[str, Any]) -> Dict[str, Any]:
    p = machine.get("payload") if isinstance(machine.get("payload"), dict) else {}
    cpu = get_nested(p, ["hardware.cpu", "cpu"], {})
    if not isinstance(cpu, dict): cpu = {}
    return {"machine": machine.get("hostname"), "machine_id": machine.get("machine_id"), "component":"CPU", "name": clean_str(_v10_pick(cpu, "name", "model", "brand", default=machine.get("cpu_name", ""))), "usage_percent": machine.get("cpu_percent", ""), "temperature_c": machine.get("cpu_temp_c", _v10_pick(cpu, "temperature_c", "temp_c", default="")), "cores": _v10_pick(cpu, "cores", "physical_cores", default=""), "threads": _v10_pick(cpu, "threads", "logical_processors", default=""), "sensor_note":"CPU temp N/A when client sensor tool/driver does not expose it"}


def _v10_usb_rows(machine: Dict[str, Any]) -> List[Dict[str, Any]]:
    p = machine.get("payload") if isinstance(machine.get("payload"), dict) else {}
    devices = normalize_usb_list(get_nested(p, ["usb.devices", "usb", "peripherals"], []))
    rows=[]
    for u in devices:
        rows.append({"machine": machine.get("hostname") or machine.get("machine_id"), "machine_id": machine.get("machine_id"), "primary_ip": machine.get("primary_ip", ""), "device": clean_str(u.get("display_name") or u.get("name")), "type": clean_str(u.get("type")), "class": clean_str(u.get("class")), "vid": clean_str(u.get("vid")), "pid": clean_str(u.get("pid")), "manufacturer": clean_str(u.get("manufacturer")), "status": clean_str(u.get("status")), "source": clean_str(u.get("source")), "device_id": clean_str(u.get("device_id"))})
    return rows


def _v10_software_rows(machine_filter: str = "") -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    for m in _v10_current_machines():
        if machine_filter and m.get("machine_id") != machine_filter:
            continue
        p = m.get("payload") if isinstance(m.get("payload"), dict) else {}
        apps = _v10_first_list(p, ["software.installed", "software.apps", "software"])
        for a in apps:
            if not isinstance(a, dict):
                continue
            rows.append({
                "machine": m.get("hostname") or m.get("machine_id"), "machine_id": m.get("machine_id"), "primary_ip": m.get("primary_ip", ""), "os": m.get("os", ""),
                "name": clean_str(_v10_pick(a, "name", "display_name", "software_name", "product_name")), "version": clean_str(_v10_pick(a, "version", "display_version")),
                "publisher": clean_str(_v10_pick(a, "publisher", "vendor", "manufacturer")), "install_date": clean_str(_v10_pick(a, "install_date", "InstallDate", "installDate")),
                "install_location": clean_str(_v10_pick(a, "install_location", "path", "location")), "source": clean_str(_v10_pick(a, "source", default="client_payload")),
                "license_review": "Review commercial/license status manually; do not store plaintext keys here",
            })
    return rows


def _v10_hardware_rows() -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    for m in _v10_current_machines():
        rows.append(_v10_cpu_row(m))
        rows.extend(_v10_memory_rows(m))
        for d in _v10_disk_rows(m):
            r = dict(d); r["component"] = "Disk/Storage"; rows.append(r)
        for g in _v10_gpu_rows(m):
            r = dict(g); r["component"] = "GPU"; rows.append(r)
        for u in _v10_usb_rows(m):
            r = dict(u); r["component"] = "USB/Peripheral"; rows.append(r)
    return rows


def _v10_machine_detail(mid: str) -> Dict[str, Any]:
    machines = _v10_current_machines()
    m = next((x for x in machines if x.get("machine_id") == mid), None) or (machines[0] if machines else {})
    if not m:
        return {"ok": False, "error": "no_machine"}
    return {"ok": True, "machine": m, "cpu": _v10_cpu_row(m), "memory": _v10_memory_rows(m), "disks": _v10_disk_rows(m), "gpus": _v10_gpu_rows(m), "usb": _v10_usb_rows(m), "software": _v10_software_rows(m.get("machine_id")), "v10_version": V10_MERGE_VERSION}


def _v10_load_sw_register() -> List[Dict[str, Any]]:
    try:
        if V10_SOFTWARE_REGISTER_PATH.exists():
            data = json.loads(V10_SOFTWARE_REGISTER_PATH.read_text(encoding="utf-8"))
...
def _v10_save_sw_register(rows: List[Dict[str, Any]]) -> None:
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    tmp = V10_SOFTWARE_REGISTER_PATH.with_suffix(".tmp")
    tmp.write_text(json.dumps(rows, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    os.replace(str(tmp), str(V10_SOFTWARE_REGISTER_PATH))


def _v10_norm_sw_asset(body: Dict[str, Any]) -> Dict[str, Any]:
    b = dict(body or {})
    name = clean_str(_v10_pick(b, "product_name", "name", "software_name"))
    seed = clean_str(b.get("id") or name or time.time())
    b["id"] = clean_str(b.get("id")) or "SWA-" + hashlib.sha1(seed.encode("utf-8", "ignore")).hexdigest()[:10].upper()
    b["product_name"] = name
    b["vendor"] = clean_str(_v10_pick(b, "vendor", "publisher"))
    b["license_type"] = clean_str(_v10_pick(b, "license_type", "type", default="Review"))
    b["license_count"] = clean_str(_v10_pick(b, "license_count", "seats", default="1"))
    b["assigned_to_employee"] = clean_str(_v10_pick(b, "assigned_to_employee", "employee", "assigned_user"))
    b["assigned_to_machine"] = clean_str(_v10_pick(b, "assigned_to_machine", "hostname", "machine"))
    b["login_username"] = clean_str(_v10_pick(b, "login_username", "username", "account"))
    b["password_vault_ref"] = clean_str(_v10_pick(b, "password_vault_ref", "vault_ref", "password_ref"))
    b["license_key_or_serial_ref"] = clean_str(_v10_pick(b, "license_key_or_serial_ref", "key_ref", "product_key_last5"))
    b["Bill/Invoice/PO No"] = clean_str(_v10_pick(b, "Bill/Invoice/PO No", "bill_invoice_po_no", "po_no", "invoice_no"))
    b["bill_invoice_po_no"] = b["Bill/Invoice/PO No"]
    b["bill_link"] = clean_str(_v10_pick(b, "bill_link", "proof_link", "invoice_link"))
    b["renewal_date"] = clean_str(_v10_pick(b, "renewal_date", "expiry_date", "subscription_end"))
    b["lifecycle_status"] = clean_str(_v10_pick(b, "lifecycle_status", "status", default="Active"))
    b["password_rule"] = "Use password vault/reference only. No plaintext password export."
    b["updated_at"] = now_iso()
    b.setdefault("created_at", b["updated_at"])
    return b


def _v10_csv(rows: List[Dict[str, Any]], filename: str) -> Tuple[bytes, Dict[str, str]]:
    return csv_response(rows, filename)


def _v10_send_csv(self: Handler, rows: List[Dict[str, Any]], filename: str) -> None:
    body, headers = _v10_csv(rows, filename)
    return self._send(200, body, "text/csv; charset=utf-8", headers)
...
def _v10_do_GET(self) -> None:
    try:
        ...
        if path.startswith("/api/v10/"):
            if not self.is_authenticated() and not is_local_request(self.client_address[0]):
                return self.send_json({"error":"login_required"}, 401)
            mid = (qs.get("machine_id") or qs.get("id") or [""])[0]
            if path == "/api/v10/status":
                return self.send_json({"ok": True, "version": V10_MERGE_VERSION, "live_db": str(DB_PATH), "sidecar_software_register": str(V10_SOFTWARE_REGISTER_PATH), "rule": "old UI kept; V10 APIs are read-only for live DB"})
            if path == "/api/v10/current-machines":
                rows = _v10_current_machines()
                return self.send_json({"ok": True, "count": len(rows), "machines": rows, "version": V10_MERGE_VERSION})
            if path == "/api/v10/machine-360":
                return self.send_json(_v10_machine_detail(mid))
            if path == "/api/v10/hardware-inventory":
                rows = _v10_hardware_rows()
                return self.send_json({"ok": True, "count": len(rows), "rows": rows})
            if path == "/api/v10/gpu-inventory":
                rows=[]
                for m in _v10_current_machines():
                    if mid and m.get("machine_id") != mid: continue
                    rows.extend(_v10_gpu_rows(m))
                return self.send_json({"ok": True, "count": len(rows), "rows": rows})
            if path == "/api/v10/usb-inventory":
                rows=[]
                for m in _v10_current_machines():
                    if mid and m.get("machine_id") != mid: continue
                    rows.extend(_v10_usb_rows(m))
                return self.send_json({"ok": True, "count": len(rows), "rows": rows})
            if path == "/api/v10/software-inventory":
                rows = _v10_software_rows(mid)
                return self.send_json({"ok": True, "count": len(rows), "rows": rows})
            if path == "/api/v10/software-register":
                rows = [_v10_norm_sw_asset(r) for r in _v10_load_sw_register()]
                return self.send_json({"ok": True, "count": len(rows), "rows": rows, "file": str(V10_SOFTWARE_REGISTER_PATH), "password_rule":"vault/reference only"})
            if path == "/api/v10/export/current_machines.csv":
                return _v10_send_csv(self, _v10_current_machines(), "V10_CURRENT_MACHINE_CONCEPT.csv")
            if path == "/api/v10/export/hardware_inventory.csv":
                return _v10_send_csv(self, _v10_hardware_rows(), "V10_HARDWARE_INVENTORY_CPU_RAM_DISK_GPU_USB.csv")
            if path == "/api/v10/export/gpu_inventory.csv":
                rows=[]
                for m in _v10_current_machines(): rows.extend(_v10_gpu_rows(m))
                return _v10_send_csv(self, rows, "V10_GPU_INVENTORY.csv")
            if path == "/api/v10/export/usb_inventory.csv":
                rows=[]
                for m in _v10_current_machines(): rows.extend(_v10_usb_rows(m))
                return _v10_send_csv(self, rows, "V10_USB_PERIPHERAL_INVENTORY.csv")
            if path == "/api/v10/export/software_inventory.csv":
                return _v10_send_csv(self, _v10_software_rows(mid), "V10_SOFTWARE_INVENTORY_FROM_CLIENTS.csv")
            if path == "/api/v10/export/software_register.csv":
                return _v10_send_csv(self, [_v10_norm_sw_asset(r) for r in _v10_load_sw_register()], "V10_SOFTWARE_LICENSE_REGISTER.csv")
            return self.send_json({"error":"v10_route_not_found", "path": path}, 404)
        return _v10_old_do_GET(self)
    except Exception as e:
        log(traceback.format_exc())
        return self.send_json({"error":"v10_server_error", "message": str(e)}, 500)
...
def _v10_do_POST(self) -> None:
    try:
        u = _v10_urlparse.urlparse(self.path)
        path = u.path
        if path.startswith("/api/v10/") and not self.is_admin() and not is_local_request(self.client_address[0]):
            return self.send_json({"error":"admin_required"}, 403)
        if path == "/api/v10/software-register/save":
            body = self.read_json()
            row = _v10_norm_sw_asset(body)
            rows = [_v10_norm_sw_asset(r) for r in _v10_load_sw_register()]
            found = False
            for i, r in enumerate(rows):
                if clean_str(r.get("id")) == clean_str(row.get("id")):
                    row["created_at"] = r.get("created_at") or row.get("created_at")
                    rows[i] = row
                    found = True
                    break
            if not found:
                rows.append(row)
            _v10_save_sw_register(rows)
            return self.send_json({"ok": True, "saved": row, "created": not found})
        if path == "/api/v10/software-register/delete":
            body = self.read_json(); rid = clean_str(body.get("id"))
            rows = [r for r in _v10_load_sw_register() if clean_str(r.get("id")) != rid]
            _v10_save_sw_register(rows)
            return self.send_json({"ok": True, "deleted_id": rid})
...
print("Loaded " + V10_MERGE_VERSION)
# ===== END V10 MERGE ADDON =====
...
def main() -> None:
    ...
    print("V10 merged read-only mode: init_db skipped; live DB not modified")
