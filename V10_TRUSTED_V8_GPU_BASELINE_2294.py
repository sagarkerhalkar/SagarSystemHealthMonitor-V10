...
def overview() -> Dict[str, Any]:
    ...
    # V10 ONLY: Dashboard Fleet Status count must come from machine registry,
    # not raw latest rows. This fixes duplicate machine_id / fake UUID / hostname-change cases.
    try:
        reg_path = DATA_DIR / "machine_registry.json"
        if reg_path.exists():
            reg = json.loads(reg_path.read_text(encoding="utf-8"))
            reg_machines = reg.get("machines") or []
            total = len(reg_machines)
            online = sum(1 for x in reg_machines if x.get("online"))
            offline = total - online
            critical = sum(1 for x in reg_machines if x.get("attention"))
    except Exception as e:
        log("V10 registry overview count fallback: " + str(e))
...
def main() -> None:
    ...
    print("V10 trusted read-only mode: init_db skipped; live DB not modified")
