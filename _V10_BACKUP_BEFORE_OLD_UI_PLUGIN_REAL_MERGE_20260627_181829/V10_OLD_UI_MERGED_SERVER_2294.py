...
V10_FIX_VERSION = "V10-fix-old-ui-real-merge-20260627b"
try:
...
def _v10_current_machines() -> List[Dict[str, Any]]:
    ...
    for m in raw:
        ...
        mid = clean_str(m.get("machine_id"))
        # Keep current-machine concept: one row per real PC. Good asset id wins; bad stale duplicate serial rows are hidden.
        if mid.startswith("ASSET:"):
            key = "asset:" + mid
        elif host and ip:
            key = "hostip:" + host.lower() + "|" + ip
        elif host:
            key = "host:" + host.lower()
        else:
            key = "id:" + mid
        ...
        item["v10_merge_version"] = V10_FIX_VERSION
        old = grouped.get(key)
        if not old or item.get("v10_data_score",0) > old.get("v10_data_score",0):
            grouped[key] = item
    out = list(grouped.values())
    out.sort(key=lambda x: (not bool(x.get("online")), clean_str(x.get("hostname")).lower()))
    return out
...
def _v10_overview() -> Dict[str, Any]:
    ...
    total=len(machines); online=sum(1 for m in machines if m.get("online")); offline=total-online
    critical=sum(1 for m in machines if (to_float(m.get("cpu_percent"),0) or 0)>=90 or (to_float(m.get("ram_percent"),0) or 0)>=90 or (to_float(m.get("disk_max_percent"),0) or 0)>=90)
    try:
        health = server_internet_health(False, False)
    except Exception as e:
        health = {"ok": False, "error": str(e)}
    return {"ok":True, "v10_merge_version":V10_FIX_VERSION, "total":total, "online":online, "offline":offline, "critical":critical, "machines":machines, "internet_health":health, "server_isp": server_public_internet_info(False)}
...
def _v10_do_GET(self) -> None:
    try:
        ...
        if path.startswith("/api/v10/"):
            ...
            if path == "/api/v10/current-machines": return self.send_json({"ok":True,"machines":_v10_current_machines(),"count":len(_v10_current_machines())})
