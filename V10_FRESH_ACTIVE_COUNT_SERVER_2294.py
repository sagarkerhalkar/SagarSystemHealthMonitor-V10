...
DB_PATH = Path(r"D:\SagarSystemHealthMonitor\data\monitor.db")
...
# ===== V10 FIX ADDON: old UI kept + real API merge + read-only safe =====
# This addon is inserted before main(). It is V10 only.
V10_FIX_VERSION = "V10-fresh-active-registry-count-fix-20260627f"
try:
    import urllib.parse as _v10_urlparse
except Exception:
    pass

# In read-only V10, do not write offline notification rows while GET APIs are loading.
def check_offline_notifications() -> None:  # type: ignore[override]
    return None


def _v10_pick(d: Any, *keys: str, default: Any = "") -> Any:
    ...
    for k in keys:
        ...
        if kk in loose and d.get(loose[kk]) not in (None, "", [], {}):
            return d.get(loose[kk])
    return default


def _v10_list_from_payload(payload: Dict[str, Any], paths: List[str]) -> List[Any]:
    for p in paths:
        val = get_nested(payload, [p], None)
        rows = listify(val)
        if rows:
            return rows
    return []
...
def _v10_quality(m: Dict[str, Any]) -> int:
    p = m.get("payload") if isinstance(m.get("payload"), dict) else {}
    score = 0
    for k in ["hostname","primary_ip","public_ip","isp_name","cpu_percent","ram_percent","disk_max_percent","wan_download_mbps","wan_upload_mbps"]:
        if m.get(k) not in (None, "", [], {}):
            score += 10
    score += min(200, int(to_float(m.get("software_count"), 0) or 0))
    score += min(160, int(to_float(m.get("usb_count"), 0) or 0) * 4)
    score += min(100, int(to_float(m.get("gpu_count"), 0) or 0) * 25)
    if isinstance(p, dict):
        if _v10_list_from_payload(p, ["software.installed", "software.apps", "software"]): score += 120
        if _v10_list_from_payload(p, ["hardware.gpus", "gpu.devices", "gpu.gpus", "gpus", "gpu"]): score += 100
        if _v10_list_from_payload(p, ["storage.disks", "hardware.disks", "disks", "storage.volumes"]): score += 100
        if _v10_list_from_payload(p, ["usb.devices", "usb", "peripherals"]): score += 80
    try:
        if bool(m.get("online")): score += 50
    except Exception:
        pass
    return score


def _v10_dt_ts(value: Any) -> float:
    try:
        txt = clean_str(value).replace("Z", "+00:00")
        if not txt:
            return 0.0
...
def _v10_norm_token(value: Any) -> str:
    return re.sub(r"[^a-z0-9]+", "", clean_str(value).lower())


def _v10_norm_mac(value: Any) -> str:
    return re.sub(r"[^A-Fa-f0-9]", "", clean_str(value)).upper()


def _v10_extract_mac(payload: Dict[str, Any]) -> str:
    try:
        mac = first_physical_mac(payload)
        if mac:
            return _v10_norm_mac(mac)
    except Exception:
        pass
    adapters = get_nested(payload, ["network.adapters", "adapters"], [])
    for a in listify(adapters):
        if not isinstance(a, dict):
            continue
        mac = _v10_norm_mac(a.get("mac") or a.get("mac_address") or a.get("MACAddress"))
        if len(mac) >= 12 and mac not in {"000000000000", "FFFFFFFFFFFF"}:
            return mac
    return ""


def _v10_machine_key(m: Dict[str, Any]) -> str:
    """Current machine concept.
    One real PC must be one row even when old DB has duplicate ASSET/MOTHERBOARD/HOSTNAME rows.
    Hostname is the primary grouping key because the live working client reports stable hostnames.
    MAC/UUID/serial/IP are fallbacks only when hostname is missing.
    """
    p = m.get("payload") if isinstance(m.get("payload"), dict) else {}
    host = _v10_norm_token(m.get("hostname") or get_nested(p, ["hostname", "identity.hostname", "computer_name"], ""))
    if host and host not in {"unknownhost", "unknown", "localhost"}:
        return "host:" + host
    mac = _v10_extract_mac(p)
    if mac:
        return "mac:" + mac
    identity = p.get("identity") if isinstance(p.get("identity"), dict) else {}
    for raw in [identity.get("system_uuid"), p.get("system_uuid"), identity.get("bios_serial"), p.get("bios_serial"), identity.get("motherboard_serial"), p.get("motherboard_serial")]:
        tok = _v10_norm_token(raw)
        if tok and tok not in {"none", "null", "defaultstring", "tobefilledbyoem", "bss0123456789"}:
            return "hw:" + tok
    ip = clean_str(m.get("primary_ip") or get_nested(p, ["network.primary_ip", "primary_ip"], ""))
    if ip:
        return "ip:" + ip
    return "id:" + clean_str(m.get("machine_id"))


def _v10_merge_values(base: Dict[str, Any], donor: Dict[str, Any]) -> Dict[str, Any]:
    """Fill empty fields in base from donor without replacing good values."""
...
def _v10_best_payload_row(items: List[Dict[str, Any]]) -> Dict[str, Any]:
    return max(items, key=lambda x: (_v10_quality(x), bool(x.get("online")), _v10_dt_ts(x.get("updated_at"))))


def _v10_newest_row(items: List[Dict[str, Any]]) -> Dict[str, Any]:
    return max(items, key=lambda x: (_v10_dt_ts(x.get("updated_at")), bool(x.get("online")), _v10_quality(x)))




def _v10_now_ts() -> float:
    try:
...
def _v10_env_float(name: str, default: float) -> float:
    try:
        import os
...
def _v10_active_window_hours() -> float:
    # Fresh V10 rule: do not count every historic row in latest table.
    # Count only machines seen in the active window unless a future registry file is added.
    return _v10_env_float("V10_ACTIVE_WINDOW_HOURS", 24.0)


def _v10_online_timeout_minutes() -> float:
    return _v10_env_float("V10_ONLINE_TIMEOUT_MINUTES", 2.0)


def _v10_is_seen_in_active_window(m: Dict[str, Any], now_ts: float) -> bool:
    ts = _v10_dt_ts(m.get("updated_at") or m.get("received_at") or m.get("last_seen"))
    if not ts:
        # keep records without time only if old server says online; otherwise they are history/noise
        return bool(m.get("online"))
    age_hours = max(0.0, (now_ts - ts) / 3600.0) if now_ts else 0.0
    return age_hours <= _v10_active_window_hours()


def _v10_force_online_status(m: Dict[str, Any], now_ts: float) -> bool:
    # Fresh current-machine concept: online must be based on latest heartbeat time, not stale duplicate identity rows.
    ts = _v10_dt_ts(m.get("updated_at") or m.get("received_at") or m.get("last_seen"))
    if ts and now_ts:
        return ((now_ts - ts) / 60.0) <= _v10_online_timeout_minutes()
    return bool(m.get("online"))

def _v10_current_machines() -> List[Dict[str, Any]]:
    raw_all = load_latest()
    now_ts = _v10_now_ts()
    ...
    for m in raw_all:
        if not isinstance(m, dict):
            continue
        if _v10_is_seen_in_active_window(m, now_ts):
            raw.append(m)

    groups: Dict[str, List[Dict[str, Any]]] = {}
    for m in raw:
        item = dict(m)
        item["v10_data_score"] = _v10_quality(item)
        item["v10_active_window_hours"] = _v10_active_window_hours()
        item["v10_online_timeout_minutes"] = _v10_online_timeout_minutes()
        key = _v10_machine_key(item)
        item["v10_current_machine_key"] = key
        groups.setdefault(key, []).append(item)

    out: List[Dict[str, Any]] = []
    for key, items in groups.items():
        # Rich row keeps software/hardware/GPU/USB payload. Newest row keeps live/last-seen status.
        rich = dict(_v10_best_payload_row(items))
        newest = _v10_newest_row(items)
        canonical = _v10_merge_values(rich, newest)
        # Status must be based on heartbeat age of newest row, not stale historic online flag.
        canonical["online"] = _v10_force_online_status(newest, now_ts)
        canonical["updated_at"] = newest.get("updated_at") or canonical.get("updated_at")
        ts = _v10_dt_ts(canonical.get("updated_at"))
        canonical["v10_last_seen_age_minutes"] = round(max(0.0, (now_ts-ts)/60.0), 2) if ts and now_ts else ""
        canonical["offline_minutes"] = canonical.get("v10_last_seen_age_minutes") if not canonical.get("online") else 0
        # Fill key visible fields from newest if rich row was old/stale.
        for k in ["hostname", "primary_ip", "public_ip", "isp_name", "wan_download_mbps", "wan_upload_mbps", "today_download_gb", "today_upload_gb", "cpu_percent", "ram_percent", "disk_max_percent"]:
            if newest.get(k) not in (None, "", [], {}):
                canonical[k] = newest.get(k)
        # Keep the rich payload for inventory; if no payload, use newest payload.
        if not isinstance(canonical.get("payload"), dict) and isinstance(newest.get("payload"), dict):
            canonical["payload"] = newest.get("payload")
        canonical["v10_duplicate_count"] = len(items)
        canonical["v10_merged_machine_ids"] = [clean_str(x.get("machine_id")) for x in items if x.get("machine_id")]
        canonical["v10_current_machine_key"] = key
        canonical["v10_merge_version"] = V10_FIX_VERSION
        out.append(canonical)

    out.sort(key=lambda x: (not bool(x.get("online")), clean_str(x.get("hostname")).lower(), clean_str(x.get("primary_ip"))))
    return out

def _v10_gpu_rows_for(m: Dict[str, Any]) -> List[Dict[str, Any]]:
    p = m.get("payload") if isinstance(m.get("payload"), dict) else {}
    raw = _v10_list_from_payload(p, ["hardware.gpus", "gpu.devices", "gpu.gpus", "gpus", "gpu"])
    if not raw and m.get("gpu_names"):
        names = m.get("gpu_names")
        raw = [{"name": n} for n in (names if isinstance(names, list) else [names])]
    rows=[]
    for i,g in enumerate(raw):
        if not isinstance(g, dict): g={"name": clean_str(g)}
        rows.append({
            "machine": m.get("hostname") or m.get("machine_id"), "machine_id": m.get("machine_id"), "primary_ip": m.get("primary_ip"),
            "gpu_index": i, "name": clean_str(_v10_pick(g,"name","display_name","caption","model","description")),
            "memory_mb": _v10_pick(g,"memory_mb","dedicated_memory_mb","adapter_ram_mb","vram_mb", default=""),
            "shared_memory_mb": _v10_pick(g,"shared_memory_mb", default=""),
            "usage_percent": _v10_pick(g,"usage_percent","utilization_percent","load_percent", default=""),
            "temperature_c": _v10_pick(g,"temperature_c","temp_c", default=""),
            "driver_version": clean_str(_v10_pick(g,"driver_version","driver", default="")),
            "source": clean_str(_v10_pick(g,"source", default="client_payload")),
            "sensor_note": clean_str(_v10_pick(g,"sensor_note","note", default="N/A when vendor/OS does not expose exact value")),
        })
    return rows


def _v10_disk_rows_for(m: Dict[str, Any]) -> List[Dict[str, Any]]:
    p = m.get("payload") if isinstance(m.get("payload"), dict) else {}
    raw = _v10_list_from_payload(p, ["storage.disks", "hardware.disks", "disks", "storage.volumes"])
    rows=[]
    for i,d in enumerate(raw):
        if not isinstance(d, dict): continue
        total = to_float(_v10_pick(d,"total_gb","size_gb","capacity_gb","total", default=""), None)
        used = to_float(_v10_pick(d,"used_gb","used", default=""), None)
        free = to_float(_v10_pick(d,"free_gb","free", default=""), None)
        pct = to_float(_v10_pick(d,"percent_used","used_percent","usage_percent","percent", default=""), None)
        if pct is None and total and used is not None:
            pct = round((used/total)*100,2)
        rows.append({
            "machine": m.get("hostname") or m.get("machine_id"), "machine_id": m.get("machine_id"), "primary_ip": m.get("primary_ip"),
            "disk": clean_str(_v10_pick(d,"name","drive","mount","letter","device", default=f"disk-{i}")),
            "model": clean_str(_v10_pick(d,"model","caption","friendly_name", default="")),
            "serial": clean_str(_v10_pick(d,"serial","serial_number", default="")),
            "type": clean_str(_v10_pick(d,"type","media_type","bus_type","kind", default="")),
            "total_gb": total if total is not None else "", "used_gb": used if used is not None else "", "free_gb": free if free is not None else "", "used_percent": pct if pct is not None else "",
            "health": clean_str(_v10_pick(d,"health","health_status","status", default="")),
            "temperature_c": _v10_pick(d,"temperature_c","temp_c","temperature", default=""),
            "firmware": clean_str(_v10_pick(d,"firmware","firmware_version", default="")),
            "sensor_note": clean_str(_v10_pick(d,"sensor_note","note", default="Disk temp/SMART is N/A when hardware/driver does not expose it")),
        })
    return rows


def _v10_usb_rows_for(m: Dict[str, Any]) -> List[Dict[str, Any]]:
    p = m.get("payload") if isinstance(m.get("payload"), dict) else {}
    devices = normalize_usb_list(get_nested(p, ["usb.devices", "usb", "peripherals"], []))
    rows=[]
    for u in devices:
        rows.append({
            "machine": m.get("hostname") or m.get("machine_id"), "machine_id": m.get("machine_id"), "primary_ip": m.get("primary_ip"),
            "device": clean_str(u.get("display_name") or u.get("name")), "type": clean_str(u.get("type")), "class": clean_str(u.get("class")),
            "vid": clean_str(u.get("vid")), "pid": clean_str(u.get("pid")), "manufacturer": clean_str(u.get("manufacturer")),
            "status": clean_str(u.get("status")), "source": clean_str(u.get("source")), "device_id": clean_str(u.get("device_id"))
        })
    return rows


def _v10_software_rows(mid: str = "") -> List[Dict[str, Any]]:
    rows=[]
    for m in _v10_current_machines():
        if mid and clean_str(m.get("machine_id")) != clean_str(mid):
            continue
        p = m.get("payload") if isinstance(m.get("payload"), dict) else {}
        apps = _v10_list_from_payload(p, ["software.installed", "software.apps", "software"])
        for a in apps:
            if not isinstance(a, dict): continue
            rows.append({
                "machine": m.get("hostname") or m.get("machine_id"), "machine_id": m.get("machine_id"), "primary_ip": m.get("primary_ip"), "os": m.get("os", ""),
                "name": clean_str(_v10_pick(a,"name","display_name","software_name","product_name")),
                "version": clean_str(_v10_pick(a,"version","display_version", default="")),
                "publisher": clean_str(_v10_pick(a,"publisher","vendor","manufacturer", default="")),
                "install_date": clean_str(_v10_pick(a,"install_date","InstallDate","installDate", default="")),
                "install_location": clean_str(_v10_pick(a,"install_location","path","location", default="")),
                "source": clean_str(_v10_pick(a,"source", default="client_payload")),
                "license_review": "manual/commercial review pending",
            })
    return rows


def _v10_hw_rows() -> List[Dict[str, Any]]:
    rows=[]
    for m in _v10_current_machines():
        p = m.get("payload") if isinstance(m.get("payload"), dict) else {}
        cpu = get_nested(p, ["hardware.cpu", "cpu"], {})
        if not isinstance(cpu, dict): cpu = {}
        rows.append({"component":"CPU", "machine":m.get("hostname"), "machine_id":m.get("machine_id"), "name":clean_str(_v10_pick(cpu,"name","model","brand", default=m.get("cpu_name", ""))), "usage_percent":m.get("cpu_percent", ""), "temperature_c":m.get("cpu_temp_c", _v10_pick(cpu,"temperature_c","temp_c", default="")), "cores":_v10_pick(cpu,"cores","physical_cores", default=""), "threads":_v10_pick(cpu,"threads","logical_processors", default=""), "sensor_note":"CPU temp N/A when exact sensor unavailable"})
        rows.append({"component":"RAM", "machine":m.get("hostname"), "machine_id":m.get("machine_id"), "total_gb":m.get("ram_total_gb", ""), "used_gb":m.get("ram_used_gb", ""), "used_percent":m.get("ram_percent", ""), "temperature_c":"", "sensor_note":"RAM temp usually not exposed by OS/motherboard"})
        for d in _v10_disk_rows_for(m):
            r=dict(d); r["component"]="Disk/Storage"; rows.append(r)
        for g in _v10_gpu_rows_for(m):
            r=dict(g); r["component"]="GPU"; rows.append(r)
        for u in _v10_usb_rows_for(m):
            r=dict(u); r["component"]="USB/Peripheral"; rows.append(r)
    return rows


def _v10_overview() -> Dict[str, Any]:
    machines = _v10_current_machines()
    total = len(machines)
    online = sum(1 for m in machines if m.get("online"))
    offline = total - online
    attention = sum(1 for m in machines if (to_float(m.get("cpu_percent"),0) or 0)>=90 or (to_float(m.get("ram_percent"),0) or 0)>=90 or (to_float(m.get("disk_max_percent"),0) or 0)>=90)
    try:
        health = server_internet_health(False, False)
    except Exception as e:
    ...
    try:
        ...
        server_isp = {"ok": False, "error": str(e)}
    # Keep many field names because old app.js versions used different names in V5/V6/V7/V8.
    return {
        "ok": True,
        "v10_merge_version": V10_FIX_VERSION,
        "current_machine_concept": "fresh active-window register: count only active-window machines, then dedupe by hostname/MAC/UUID/serial/IP",
        "active_window_hours": _v10_active_window_hours(),
        "online_timeout_minutes": _v10_online_timeout_minutes(),
        "total": total,
        ...
        "critical": attention,
        "attention": attention,
        "attention_machines": attention,
        "critical_machines": attention,
        "summary": {"total": total, "total_machines": total, "online": online, "online_machines": online, "offline": offline, "offline_machines": offline, "attention": attention, "critical": attention},
        "machines": machines,
        "internet_health": health,
        "server_isp": server_isp,
    }

def _v10_csv_send(self: Handler, rows: List[Dict[str, Any]], filename: str) -> None:
    body, headers = csv_response(rows, filename)
    return self._send(200, body, "text/csv; charset=utf-8", headers)


_v10_old_do_GET = Handler.do_GET
_v10_old_do_POST = Handler.do_POST


def _v10_do_GET(self) -> None:
    try:
        u = _v10_urlparse.urlparse(self.path)
        path = u.path
        qs = _v10_urlparse.parse_qs(u.query)
        mid = (qs.get("machine_id") or qs.get("id") or [""])[0]
        # Merge into OLD UI endpoints, so old app.js stops spinning and receives improved current-machine data.
        if path == "/api/overview":
            return self.send_json(_v10_overview())
        if path == "/api/machines":
            return self.send_json({"machines": _v10_current_machines(), "v10_merge_version": V10_FIX_VERSION})
        if path == "/api/machine":
            machines = _v10_current_machines()
            found = next((m for m in machines if clean_str(m.get("machine_id")) == clean_str(mid)), None)
            return self.send_json(found or {"error":"Machine not found", "machine_id": mid}, 200 if found else 404)
        if path == "/api/export/machine_current.csv":
            rows = _v10_current_machines()
            if mid: rows=[m for m in rows if clean_str(m.get("machine_id")) == clean_str(mid)]
            return _v10_csv_send(self, rows, "V10_current_machines.csv")
        if path == "/api/export/software.csv":
            return _v10_csv_send(self, _v10_software_rows(mid), "V10_software_inventory.csv")
        if path == "/api/export/usb.csv":
            rows=[]
            for m in _v10_current_machines():
                if mid and clean_str(m.get("machine_id")) != clean_str(mid): continue
                rows.extend(_v10_usb_rows_for(m))
            return _v10_csv_send(self, rows, "V10_usb_inventory.csv")
        if path == "/api/export/gpu.csv":
            rows=[]
            for m in _v10_current_machines():
                if mid and clean_str(m.get("machine_id")) != clean_str(mid): continue
                rows.extend(_v10_gpu_rows_for(m))
            return _v10_csv_send(self, rows, "V10_gpu_inventory.csv")
        if path == "/api/export/hardware.csv":
            return _v10_csv_send(self, _v10_hw_rows(), "V10_hardware_inventory.csv")
        if path.startswith("/api/v10/"):
            if path == "/api/v10/status": return self.send_json({"ok":True,"version":V10_FIX_VERSION,"live_db":str(DB_PATH),"ui":"old 2278 UI kept","active_window_hours":_v10_active_window_hours(),"online_timeout_minutes":_v10_online_timeout_minutes()})
            if path == "/api/v10/count-debug":
                raw_all=load_latest(); now_ts=_v10_now_ts(); raw=[m for m in raw_all if isinstance(m,dict) and _v10_is_seen_in_active_window(m, now_ts)]; cm=_v10_current_machines(); return self.send_json({"ok":True,"version":V10_FIX_VERSION,"raw_latest_rows_all_time":len(raw_all),"raw_rows_inside_active_window":len(raw),"current_machines_after_dedup":len(cm),"online":sum(1 for m in cm if m.get("online")),"offline":sum(1 for m in cm if not m.get("online")),"active_window_hours":_v10_active_window_hours(),"online_timeout_minutes":_v10_online_timeout_minutes(),"sample":[{"hostname":m.get("hostname"),"machine_id":m.get("machine_id"),"updated_at":m.get("updated_at"),"online":m.get("online"),"age_min":m.get("v10_last_seen_age_minutes"),"key":m.get("v10_current_machine_key"),"dup":m.get("v10_duplicate_count")} for m in cm[:200]]})
            if path == "/api/v10/current-machines":
                cm=_v10_current_machines(); return self.send_json({"ok":True,"machines":cm,"count":len(cm),"total":len(cm),"online":sum(1 for m in cm if m.get("online")),"offline":sum(1 for m in cm if not m.get("online")),"version":V10_FIX_VERSION})
            if path == "/api/v10/dedup-report":
                cm=_v10_current_machines(); return self.send_json({"ok":True,"version":V10_FIX_VERSION,"total":len(cm),"online":sum(1 for m in cm if m.get("online")),"offline":sum(1 for m in cm if not m.get("online")),"machines":[{"hostname":m.get("hostname"),"machine_id":m.get("machine_id"),"key":m.get("v10_current_machine_key"),"duplicate_count":m.get("v10_duplicate_count"),"merged_ids":m.get("v10_merged_machine_ids"),"online":m.get("online"),"updated_at":m.get("updated_at"),"score":m.get("v10_data_score")} for m in cm]})
            if path == "/api/v10/software-inventory": return self.send_json({"ok":True,"rows":_v10_software_rows(mid),"count":len(_v10_software_rows(mid))})
            if path == "/api/v10/gpu-inventory":
                rows=[]
                for m in _v10_current_machines(): rows.extend(_v10_gpu_rows_for(m))
                return self.send_json({"ok":True,"rows":rows,"count":len(rows)})
            if path == "/api/v10/hardware-inventory": return self.send_json({"ok":True,"rows":_v10_hw_rows(),"count":len(_v10_hw_rows())})
            if path == "/api/v10/usb-inventory":
                rows=[]
                for m in _v10_current_machines(): rows.extend(_v10_usb_rows_for(m))
                return self.send_json({"ok":True,"rows":rows,"count":len(rows)})
        return _v10_old_do_GET(self)
    except Exception as e:
        log(traceback.format_exc())
        return self.send_json({"error":"v10_fix_server_error","message":str(e)},500)


def _v10_do_POST(self) -> None:
    try:
        path = self.path.split("?",1)[0]
        if path in ("/api/heartbeat", "/heartbeat", "/submit"):
            return self.send_json({"ok":False,"read_only":True,"message":"V10 is dashboard only. Clients must post to live 2278."}, 409)
        return _v10_old_do_POST(self)
    except Exception as e:
        log(traceback.format_exc())
        return self.send_json({"error":"v10_fix_server_error","message":str(e)},500)

Handler.do_GET = _v10_do_GET  # type: ignore[assignment]
Handler.do_POST = _v10_do_POST  # type: ignore[assignment]
print("Loaded " + V10_FIX_VERSION)
# ===== END V10 FIX ADDON =====
...
def main() -> None:
    ...
    print("V10 read-only: init_db skipped; live DB not modified")
