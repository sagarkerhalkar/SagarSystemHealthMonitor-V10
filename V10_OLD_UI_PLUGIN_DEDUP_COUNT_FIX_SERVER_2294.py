...
V10_FIX_VERSION = "V10-old-ui-plugin-dedup-count-fix-20260627e"
try:
...
def _v10_current_machines() -> List[Dict[str, Any]]:
    ...
    for key, items in groups.items():
        ...
        # Status must be based on any current row for the real PC, not stale duplicate rows.
        canonical["online"] = any(bool(x.get("online")) for x in items)
        canonical["updated_at"] = newest.get("updated_at") or canonical.get("updated_at")
        canonical["offline_minutes"] = min([to_float(x.get("offline_minutes"), 999999) or 999999 for x in items] or [999999])
...
def _v10_overview() -> Dict[str, Any]:
    ...
    return {
        ...
        "current_machine_concept": "dedupe by hostname first, then MAC/UUID/serial/IP; rich payload merged with newest status",
        "total": total,
...
def _v10_do_GET(self) -> None:
    try:
        ...
        if path.startswith("/api/v10/"):
            if path == "/api/v10/status": return self.send_json({"ok":True,"version":V10_FIX_VERSION,"live_db":str(DB_PATH),"ui":"old 2278 UI kept"})
