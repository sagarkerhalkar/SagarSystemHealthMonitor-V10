...
const quietPages = new Set(['machine360','network','hardware','software','usb','changes','history','deploy','settings','messages','notifications','hwinventory','swinventory','isoaudit']);
...
function shortId(id){id=String(id||''); if(!id)return''; return id.length>80?id.slice(0,80)+'Ã¢â‚¬Â¦':id;}
...
/* === NATIVE INVENTORY DASHBOARD MERGE START === */
let invHwRows = [];
let invSwLicRows = [];
const invHwCols = [
 ['Asset Code','asset_code'],['Make','make_name'],['Model','model_name'],['Asset','asset_name'],
 ['Category','asset_type'],['Details','configuration_details'],['Qty','quantity'],['Vendor','vendor_name'],
 ['Warranty End','warranty_end_date'],['Purchase Date','purchase_date'],['Bill/PO','po_invoice_bill_no'],
 ['Tag/Host','tagname_hostname'],['Serial','serial_number'],['Person','assigned_to'],['Room','asset_location'],
 ['Status','status'],['Remarks','remarks']
];
const invHwEdit = ['asset_uid','asset_code','make_name','model_name','asset_name','asset_type','configuration_details','quantity','rate','vendor_name','warranty_end_date','warranty_end_year','purchase_date','po_invoice_bill_no','po_invoice_bill_path','tagname_hostname','serial_number','assigned_to','asset_location','status','remarks'];
const invSwEdit = ['license_uid','software_name','vendor_name','publisher','version','license_type','license_count','assigned_to','assigned_machine','login_username','password_vault_ref','license_key_ref','purchase_date','renewal_date','expiry_date','po_invoice_bill_no','po_invoice_bill_path','status','remarks'];

async function invJson(url,opt){
  const r = await fetch(url,opt);
  if(!r.ok) throw new Error(await r.text());
  return await r.json();
}
function invCard(k,v){
  return `<div class="native-inv-card"><div class="native-inv-k">${esc(k)}</div><div class="native-inv-v">${esc(v)}</div></div>`;
}
function invFillSelect(id,label,items){
  const el = $(id); if(!el) return;
  const old = el.value || '';
  el.innerHTML = `<option value="">${esc(label)}</option>` + (items||[]).map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('');
  if(old) el.value = old;
}
async function renderHwInventory(){
  if(state.page !== 'hwinventory') return;
  const cards = $('#hwiCards'); if(!cards) return;
  const s = await invJson('/api/inv30/hw/summary');
  cards.innerHTML =
    invCard('Assets',s.assets) + invCard('Missing Vendor',s.missing_vendor) +
    invCard('Missing Make',s.missing_make) + invCard('Missing Serial',s.missing_serial) +
    invCard('Missing Tag/Host',s.missing_tag) + invCard('Missing Person',s.missing_person) +
    invCard('Missing Room',s.missing_room) + invCard('Missing Bill/PO',s.missing_bill);
  invFillSelect('#hwiCategory','Category',s.categories);
  invFillSelect('#hwiRoom','Room',s.rooms);
  invFillSelect('#hwiPerson','Person',s.persons);
  invFillSelect('#hwiVendor','Vendor',s.vendors);
  invFillSelect('#hwiStatus','Status',s.statuses);
  await invHwLoad();
}
async function invHwLoad(){
  const p = new URLSearchParams();
  [['q','#hwiQ'],['category','#hwiCategory'],['room','#hwiRoom'],['person','#hwiPerson'],['vendor','#hwiVendor'],['status','#hwiStatus']]
    .forEach(([k,id])=>{ const v=$(id)?.value||''; if(v) p.set(k,v); });
  const j = await invJson('/api/inv30/hw/assets?' + p.toString());
  invHwRows = j.rows || [];
  const t = $('#hwiTable'); if(!t) return;
  t.querySelector('thead').innerHTML = '<tr><th class="native-inv-action">Action</th>' + invHwCols.map(c=>`<th>${esc(c[0])}</th>`).join('') + '</tr>';
  t.querySelector('tbody').innerHTML = invHwRows.map((r,i)=>`
    <tr>
      <td class="native-inv-action"><button class="btn small" onclick="invHwOpen(invHwRows[${i}])">Edit</button><br><button class="btn small danger" onclick="invHwDelete(${i})">Delete</button></td>
      ${invHwCols.map(c=>`<td>${esc(r[c[1]]||'')}</td>`).join('')}
    </tr>`).join('') || '<tr><td>No rows</td></tr>';
}
function invHwOpen(r={}){
  const f = $('#hwiForm'); if(!f) return;
  f.style.display='block';
  f.innerHTML = `<h3>${r.asset_uid?'Edit Asset':'Add Asset'}</h3><div class="native-inv-grid">` +
    invHwEdit.map(k=>`<label>${esc(k.replaceAll('_',' ').toUpperCase())}<input id="hwi_f_${k}" value="${esc(r[k]||'')}"></label>`).join('') +
    `</div><div class="native-inv-toolbar"><button class="btn primary" onclick="invHwSave()">Save</button><button class="btn" onclick="$('#hwiForm').style.display='none'">Cancel</button></div>`;
  f.scrollIntoView({behavior:'smooth',block:'start'});
}
async function invHwSave(){
  const r = {};
  invHwEdit.forEach(k=>r[k]=$(`#hwi_f_${k}`)?.value||'');
  await invJson('/api/inv30/hw/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(r)});
  $('#hwiForm').style.display='none';
  await renderHwInventory();
}
async function invHwDelete(i){
  if(!confirm('Delete this hardware asset?')) return;
  await invJson('/api/inv30/hw/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({asset_uid:invHwRows[i].asset_uid})});
  await renderHwInventory();
}

async function renderSwInventory(){
  if(state.page !== 'swinventory') return;
  const cards = $('#swiCards'); if(!cards) return;
  const s = await invJson('/api/inv30/sw/summary');
  cards.innerHTML = invCard('License Rows',s.license_rows) + invCard('Live Software Rows',s.live_software_rows);
  await invSwLoad();
}
async function invSwLoad(){
  const q = encodeURIComponent($('#swiQ')?.value||'');
  const lic = await invJson('/api/inv30/sw/licenses?q=' + q);
  invSwLicRows = lic.rows || [];
  const lt = $('#swiLicenseTable');
  const lc = ['software_name','vendor_name','version','license_type','license_count','assigned_to','assigned_machine','po_invoice_bill_no','status'];
  if(lt){
    lt.querySelector('thead').innerHTML = '<tr><th class="native-inv-action">Action</th>' + lc.map(x=>`<th>${esc(x)}</th>`).join('') + '</tr>';
    lt.querySelector('tbody').innerHTML = invSwLicRows.map((r,i)=>`
      <tr><td class="native-inv-action"><button class="btn small" onclick="invSwOpen(invSwLicRows[${i}])">Edit</button><br><button class="btn small danger" onclick="invSwDelete(${i})">Delete</button></td>${lc.map(x=>`<td>${esc(r[x]||'')}</td>`).join('')}</tr>`).join('') || '<tr><td>No license rows</td></tr>';
  }
  const live = await invJson('/api/inv30/sw/live');
  const vt = $('#swiLiveTable');
  const vc = ['hostname','ip','software_name','publisher','version','install_date'];
  if(vt){
    vt.querySelector('thead').innerHTML = '<tr>' + vc.map(x=>`<th>${esc(x)}</th>`).join('') + '</tr>';
    vt.querySelector('tbody').innerHTML = (live.rows||[]).slice(0,1000).map(r=>`<tr>${vc.map(x=>`<td>${esc(r[x]||'')}</td>`).join('')}</tr>`).join('') || '<tr><td>No live software rows</td></tr>';
  }
}
function invSwOpen(r={}){
  const f = $('#swiForm'); if(!f) return;
  f.style.display='block';
  f.innerHTML = `<h3>${r.license_uid?'Edit License':'Add License'}</h3><div class="native-inv-grid">` +
    invSwEdit.map(k=>`<label>${esc(k.replaceAll('_',' ').toUpperCase())}<input id="swi_f_${k}" value="${esc(r[k]||'')}"></label>`).join('') +
    `</div><div class="native-inv-toolbar"><button class="btn primary" onclick="invSwSave()">Save</button><button class="btn" onclick="$('#swiForm').style.display='none'">Cancel</button></div>`;
  f.scrollIntoView({behavior:'smooth',block:'start'});
}
async function invSwSave(){
  const r = {};
  invSwEdit.forEach(k=>r[k]=$(`#swi_f_${k}`)?.value||'');
  await invJson('/api/inv30/sw/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(r)});
  $('#swiForm').style.display='none';
  await renderSwInventory();
}
async function invSwDelete(i){
  if(!confirm('Delete this software license row?')) return;
  await invJson('/api/inv30/sw/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({license_uid:invSwLicRows[i].license_uid})});
  await renderSwInventory();
}

async function renderIsoAudit(){
  if(state.page !== 'isoaudit') return;
  const cards = $('#isoCards'); if(!cards) return;
  const s = await invJson('/api/inv30/iso/summary');
  cards.innerHTML =
    invCard('H/W Assets',s.hardware_assets) + invCard('S/W License Rows',s.software_license_rows) +
    invCard('Live Software',s.live_software_rows) + invCard('H/W Gaps',s.hardware_gap_rows) +
    invCard('Duplicates',s.duplicate_rows) + invCard('Warranty Issues',s.warranty_issue_rows);
}
/* === NATIVE INVENTORY DASHBOARD MERGE END === */

function renderAll(){ renderDeployCommands();  renderDashboard(); renderFleet(); renderMachine360(); renderNetwork(); renderHardware(); renderSoftware(); renderHwInventory(); renderSwInventory(); renderIsoAudit(); renderUsb(); if(state.page==='changes') renderChanges(false); if(state.page==='history') renderHistory(); if(state.page==='messages') renderMessages(); }
function switchPage(page){ state.page=page; $$('.page').forEach(p=>p.classList.remove('active')); $('#page-'+page)?.classList.add('active'); $$('.nav').forEach(n=>n.classList.toggle('active',n.dataset.page===page)); const titles={hwinventory:['H/W Inventory','Hardware asset register and ISO/ITAM evidence'],swinventory:['S/W Inventory','Live software and license register'],isoaudit:['ISO Audit','Audit evidence downloads'],dashboard:['Command Center','Colorful system-wise command analysis, ISP health, downloads, alerts and history.'],fleet:['Machine Fleet','All Windows and Ubuntu systems, stable and searchable.'],machine360:['Machine 360','Select one system and export its current details.'],network:['Network + VPN','LAN, VLAN, Wi-Fi, virtual adapters and VPN visibility.'],hardware:['Hardware Analytics','CPU, RAM, disk, temperature and GPU inventory.'],software:['Software Inventory','System-wise installed applications with admin export.'],usb:['USB + Peripherals','Human-readable keyboard, mouse, headset, camera, storage and USB network devices.'],changes:['Human Change Log','Readable system-wise timeline for USB, software, hardware, IP and VPN changes.'],history:['Day History','Old day data with system-wise download/upload and exports.'],messages:['Client Messages','Send closeable popup messages to Windows and Ubuntu clients.'],notifications:['Notifications','Create, edit, delete and test alert rules.'],deploy:['Deploy','Copy-ready current commands for Windows and Ubuntu clients.'],settings:['Settings','Users, password and refresh control.']}; const [t,sub]=titles[page]||titles.dashboard; $('#pageTitle').textContent=t; $('#pageSubtitle').textContent=sub; showBanner(state.pendingUpdate && quietPages.has(page)); renderAll(); if(page==='notifications') loadRules(); if(page==='messages') renderMessages(); if(page==='changes') renderChanges(true); if(page==='settings' && isAdmin()) loadUsers(); applyRoleControls(); }
...
// === RUNTIME DUPLICATE INVENTORY NAV CLEANUP ===
(function(){
  function cleanDuplicateInventoryNav(){
    const labels = ["H/W Inventory","S/W Inventory","ISO Audit"];
    labels.forEach(label=>{
      const items = Array.from(document.querySelectorAll(".nav, a, button, div"))
        .filter(el => (el.textContent || "").trim() === label);
      if(items.length <= 1) return;

      let keep = items.find(el => el.dataset && (
        el.dataset.page === "hwinventory" ||
        el.dataset.page === "swinventory" ||
        el.dataset.page === "isoaudit"
      )) || items[0];
  ...
  if(document.readyState === "loading"){
    document.addEventListener("DOMContentLoaded", cleanDuplicateInventoryNav);
  }else{
    cleanDuplicateInventoryNav();
  }

  window.addEventListener("load", cleanDuplicateInventoryNav);
  setInterval(cleanDuplicateInventoryNav, 1000);
})();
...
// === FORCE LIVE REFRESH ON FOR ONLINE DETECTION ===
(function(){
  function forceLiveOn(){
    try{
      if(typeof state !== "undefined"){
        state.autoRefresh = true;
      }
      const btn = document.querySelector("#settingsAutoBtn");
      if(btn && btn.textContent.includes("Paused")){
        btn.textContent = "Live Refresh: On";
      }
      const topBtns = Array.from(document.querySelectorAll("button,.btn"));
      topBtns.forEach(b=>{
        if((b.textContent || "").trim() === "Live: Paused"){
          b.textContent = "Live: On";
        }
      });
    }catch(e){}
  }
  ...
  if(document.readyState === "loading"){
    document.addEventListener("DOMContentLoaded", forceLiveOn);
  }else{
    forceLiveOn();
  }

  window.addEventListener("load", forceLiveOn);
  setInterval(forceLiveOn, 3000);
})();
...
// === HISTORY CACHE LITE UI ===
(function(){
  if(window.__historyCacheLiteUI) return;
  window.__historyCacheLiteUI = true;

  function hqs(){
    const p = new URLSearchParams();
    const df = document.querySelector("#historyFrom,#dateFrom,#historyDateFrom")?.value || "";
    const dt = document.querySelector("#historyTo,#dateTo,#historyDateTo")?.value || "";
    const mid = document.querySelector("#historyMachine")?.value || "";
  ...
  function safe(x){
    try { return esc(x ?? ""); } catch(e) { return String(x ?? ""); }
  }

  async function renderHistoryLite(){
    if(typeof state !== "undefined" && state.page !== "history") return;

    const daily = document.querySelector("#historyDailyTable tbody");
    const mt = document.querySelector("#historyMachineTable tbody");
    const st = document.querySelector("#historySampleTable tbody");
    if(!daily && !mt && !st) return;

    let d;
    try{
      const r = await fetch("/api/history-cache-lite/view?" + hqs(), {cache:"no-store"});
      d = await r.json();
      if(!d.ok) throw new Error(d.error || "History cache error");
    }catch(e){
      if(daily) daily.innerHTML = `<tr><td colspan="11">History cache loading: ${safe(e.message)}</td></tr>`;
      return;
    }

    const status = d.status || {};
    const note = `Cache ${status.percent || 0}% ready.`;

    if(daily){
      daily.innerHTML = (d.daily || []).map(x =>
        `<tr><td>${safe(x.date)}</td><td>${safe(x.machines)}</td><td>${safe(x.heartbeats)}</td><td>${safe(x.day_download_gb)}</td><td>${safe(x.day_upload_gb)}</td><td>${safe(x.max_down_mbps)}</td><td>${safe(x.max_up_mbps)}</td><td>${safe(x.avg_cpu)}</td><td>${safe(x.avg_ram)}</td><td>${safe(x.usb_max)}</td><td>${safe(x.apps_max)}</td></tr>`
      ).join("") || `<tr><td colspan="11">${safe(note)} Building history cache...</td></tr>`;
    }

    if(mt){
      mt.innerHTML = (d.per_machine || []).map(x =>
        `<tr><td>${safe(x.date)}</td><td>${safe(x.hostname || x.machine_id)}</td><td>${safe(x.heartbeats)}</td><td></td><td></td><td>${safe(x.download_gb)}</td><td>${safe(x.upload_gb)}</td><td>${safe(x.max_down_mbps)}</td><td>${safe(x.max_up_mbps)}</td><td>${safe(x.cpu_max)}</td><td>${safe(x.ram_max)}</td><td>${safe(x.ram_total_gb)}</td><td>${safe(x.usb_max)}</td><td>${safe(x.apps_max)}</td><td>${safe(x.last_seen)}</td></tr>`
      ).join("") || `<tr><td colspan="15">${safe(note)}</td></tr>`;
    }

    if(st){
      st.innerHTML = `<tr><td colspan="20">${safe(note)} Raw heartbeat sample hidden for speed.</td></tr>`;
    }
  }

  function wireLoadOnly(){
  ...
  function install(){
    if(typeof renderHistory === "function" && !window.__oldHistoryBeforeLite){
      window.__oldHistoryBeforeLite = renderHistory;
      renderHistory = renderHistoryLite;
      return true;
    }
    return false;
  }
  ...
  window.addEventListener("load", ()=>{
    setTimeout(wireLoadOnly, 800);
    setTimeout(renderHistoryLite, 1000);
  });

  setInterval(()=>{
    if(typeof state !== "undefined" && state.page === "history"){
      wireLoadOnly();
    }
  },1500);
})();
...
/* === V10_LOCK_SINGLE_CPU_RAM_RULES_UI_START === */
function v10LockSingleCpuRamRuleRows(){
  try{
    document.querySelectorAll('table tr').forEach(tr=>{
      const txt=(tr.innerText||'').toLowerCase();
      if(txt.includes('cpu_high') || txt.includes('ram_high') || txt.includes('cpu usage high (locked') || txt.includes('ram usage high (locked')){
        tr.style.opacity='0.72';
        tr.style.background='rgba(255,245,220,0.55)';
        tr.querySelectorAll('button').forEach(btn=>{
          const b=(btn.innerText||'').toLowerCase();
          if(b.includes('edit') || b.includes('delete')){
            btn.disabled=true;
            btn.title='Locked disabled. Use CPU + RAM both critical rule.';
            btn.style.opacity='0.45';
            btn.style.cursor='not-allowed';
          }
        });
        if(!tr.querySelector('.v10-locked-badge')){
          const td=document.createElement('td');
          td.className='v10-locked-badge';
          td.innerHTML='<span style="display:inline-block;padding:4px 8px;border-radius:999px;background:#fff3cd;color:#8a5a00;font-weight:800;font-size:11px;">LOCKED</span>';
          tr.appendChild(td);
        }
      }
    });
...
setInterval(v10LockSingleCpuRamRuleRows, 1200);
/* === V10_LOCK_SINGLE_CPU_RAM_RULES_UI_END === */
